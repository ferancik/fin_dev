...
class Spravajazykovweb extends CI_Controller {

    public $relativeLanduageDir = '../../language/web/';
    ...
    public function __construct() {
        ...
        $this->load->model('admin/admin_web_language_m', '', TRUE);
    }
    ...
    public function index() {
        if (!$this->tank_auth->is_logged_in() && !$this->tank_auth->is_admin()) {
            ...
            $files = null;
            $files = $this->admin_web_language_m->getActiveLanguage();
            $dblanguage = $this->admin_web_language_m->getAllLanguage();
            $this->template_admin->load('admin/template', 'admin/spravajazykovweb/index', array('alljazyky' => $files, 'dblanguage' => $dblanguage, 'subory' => $subory));
        }
    }
    ...
    public function deactive($idJazyk) {
        if (!$this->tank_auth->is_logged_in() && !$this->tank_auth->is_admin()) {
            ...
            $jazyk = $this->admin_web_language_m->getOneLanguage($idJazyk);
            if ($jazyk->dir != 'english' && $jazyk->default != 1 ) {
                $jazyk->active = 0;
                $this->admin_web_language_m->updateLanguage($jazyk->id, $jazyk);
                $this->session->set_flashdata('sprava', "info|Jazyk bol deaktivavany");
                redirect('admin/spravajazykovweb');
            } else {
    ...
    public function uprava() {
        if (!$this->tank_auth->is_logged_in() && !$this->tank_auth->is_admin()) {
            ...
            if (!$_POST) {
                if ($jazyk != 'english') {
                    $tempData = $this->admin_web_language_m->getOneLanguageDir($jazyk);
            ...
            if ($_POST['jazyk'] != '' && $_POST['nova_polozka'] != '' && $_POST['subor'] != '' && $_POST['nova_polozka_text'] != '') {
                ...
                redirect(site_url('admin/spravajazykovweb/uprava/') . '/?jazyk=' . $jazyk . '/' . $subor);
            }
            ...
            $this->template_admin->load('admin/template', 'admin/spravajazykovweb/uprava', array('jazyk' => $jazyk, 'subor' => $subor . '.php', 'lang' => $lang_edit, 'langen' => $langen));
        }
    }
    ...
    private function repairDatabaseLanguage($idLanguage) {
       ...
       foreach ($this->config->item('checklanguage') as $value) {
          $this->repairdatabaselanguage_m->check_language($value['table'],$value['colum']); 
       }
