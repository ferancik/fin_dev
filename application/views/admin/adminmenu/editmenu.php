...
<div class="modal-body">
    <?
    $action = 'admin/adminmenu/saveForm/' . (($data) ? $data->id : "");
    echo form_open($action, 'id="editMenu" class="form-horizontal" method="post"');
    ?>
    <input type="hidden" name="duplikovat" id="duplikovatMenu<?=(($data) ? $data->id : "")?>" value="0"/>
<p>
    ...
    <div id="myTabContent_<?=(($data) ? $data->id : "")?>" class="tab-content">
        <? foreach ($alljazyky as $value) { ?>
            <div id="jazyk_<?=(($data) ? $data->id : "")?>_<?= $value->id; ?>" class="tab-pane fade" >
                <?
                if ($data){
                $tempJ = $this->admin_menu_m->getMenuLanguage($data->id, $value->id);
...
<p>
   ...
   <?
    ...
    ?>
    Kontroler:
    <input type="text" name="url" id="url" class="span4" placeholder="URL" value="<?php echo $valueT; ?>"  />
...
<p>
   Skryte menu:
   <?php 
    $js = 'id="kontolneMenu" class="span4 validate[required]"';
    if (isset($data->zobrazit)){
        $selskryte = $data->zobrazit;
    }else{
        $selskryte = 1;
    }
     echo form_dropdown('kontolneMenu', array('1'=>'Nie', '0'=>'Ano'), $selskryte, $js);
...
<div class="input-append">
    ...
    <a class="btn" href="#chooseIcon" data-toggle="modal"> Vybrat ikonu</a>
...
<div class="modal-footer">
    ...
    <a class="btn " id="duplikovatMenuTlacidlo<?=(($data) ? $data->id : "")?>" >Zapnut duplikovanie</a>
...
<script type="text/javascript">
     ...
     $("#duplikovatMenuTlacidlo<?=(($data) ? $data->id : "")?>").click(function() {
       var co = $('#duplikovatMenu<?=(($data) ? $data->id : "")?>').val();
       if (co == 0){
          $('#duplikovatMenu<?=(($data) ? $data->id : "")?>').val('1');
          $("#duplikovatMenuTlacidlo<?=(($data) ? $data->id : "")?>").html('Vypnut duplikovanie');
       }else if (co == 1){
           $('#duplikovatMenu<?=(($data) ? $data->id : "")?>').val('0'); 
            $("#duplikovatMenuTlacidlo<?=(($data) ? $data->id : "")?>").html('Zapnut duplikovanie');
       }
     });
