...
class Tank_auth_groups extends Tank_auth {
    ...
    function __construct() {
        ...
        $this->ci->load->model('admin/tank_auth/ta_groups_users', 'ta_groups_users');
    ...
    function is_group_member($group_id) {
        return $this->ci->session->userdata('kfadmin_group_id') === $group_id;
    }
     ...
     */
    function is_admin() {
        return $this->ci->session->userdata('kfadmin_group_id') === '100';
    }
    ...
    function login($login, $password, $remember, $login_by_username, $login_by_email) {
        ...
        if ($loggedIn) {
            ...
            $this->ci->session->set_userdata(array('kfadmin_group_id' => $user->group_id));
        }
