...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->config('admin/tank_auth', TRUE);

		$this->ci->load->library('session');
		$this->ci->load->database();
		$this->ci->load->model('admin/tank_auth/users');
		...
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				$hasher = new PasswordHash(
						$this->ci->config->item('phpass_hash_strength', 'admin/tank_auth'),
						$this->ci->config->item('phpass_hash_portable', 'admin/tank_auth'));
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						$this->ci->session->set_userdata(array(
								'kfadmin_user_id'	=> $user->id,
								'kfadmin_username'	=> $user->username,
								'kfadmin_status'	=> ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
						));
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							$this->ci->users->update_login_info(
									$user->id,
									$this->ci->config->item('login_record_ip', 'admin/tank_auth'),
									$this->ci->config->item('login_record_time', 'admin/tank_auth'));
							return TRUE;
						}
					}
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('kfadmin_user_id' => '', 'kfadmin_username' => '', 'kfadmin_status' => ''));
	...
	{
		return $this->ci->session->userdata('kfadmin_status') === ($activated ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
	}
	...
	{
		return $this->ci->session->userdata('kfadmin_user_id');
	}
	...
	{
		return $this->ci->session->userdata('kfadmin_username');
	}
	...
	{
		$user_id = $this->ci->session->userdata('kfadmin_user_id');
	...
	{
		$this->ci->users->purge_na($this->ci->config->item('email_activation_expire', 'admin/tank_auth'));
	...
	function can_reset_password($user_id, $new_pass_key)
	{
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0)) {
			return $this->ci->users->can_reset_password(
				$user_id,
				$new_pass_key,
				$this->ci->config->item('forgot_password_expire', 'admin/tank_auth'));
		}
		return FALSE;
	}
		...
		if ((strlen($user_id) > 0) AND (strlen($new_pass_key) > 0) AND (strlen($new_password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
				...
				if ($this->ci->users->reset_password(
						...
						$hashed_password,
						$new_pass_key,
						$this->ci->config->item('forgot_password_expire', 'admin/tank_auth'))) {	// success

					// Clear all user's autologins
					$this->ci->load->model('admin/tank_auth/user_autologin');
	...
	private function create_autologin($user_id)
	{
		...
		if ($this->ci->user_autologin->set($user_id, md5($key))) {
			set_cookie(array(
					'name' 		=> $this->ci->config->item('autologin_cookie_name', 'admin/tank_auth'),
					'value'		=> serialize(array('user_id' => $user_id, 'key' => $key)),
					'expire'	=> $this->ci->config->item('autologin_cookie_life', 'admin/tank_auth'),
			));
			return TRUE;
		}
		return FALSE;
	...
	{
		$this->ci->load->helper('cookie');
		if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'admin/tank_auth'), TRUE)) {
			...
			delete_cookie($this->ci->config->item('autologin_cookie_name', 'admin/tank_auth'));
		}
	}
				...
				if (isset($data['key']) AND isset($data['user_id'])) {
					...
					if (!is_null($user = $this->ci->user_autologin->get($data['user_id'], md5($data['key'])))) {
						...
						$this->ci->session->set_userdata(array(
								'kfadmin_user_id'	=> $user->id,
								'kfadmin_username'	=> $user->username,
								'kfadmin_status'	=> STATUS_ACTIVATED,
						));
	...
	{
		if ($this->ci->config->item('login_count_attempts', 'admin/tank_auth')) {
			$this->ci->load->model('admin/tank_auth/login_attempts');
			return $this->ci->login_attempts->get_attempts_num($this->ci->input->ip_address(), $login)
					>= $this->ci->config->item('login_max_attempts', 'admin/tank_auth');
		}
		return FALSE;
	}
	...
	private function clear_login_attempts($login)
	{
		if ($this->ci->config->item('login_count_attempts', 'admin/tank_auth')) {
			...
			$this->ci->login_attempts->clear_attempts(
					...
					$login,
					$this->ci->config->item('login_attempt_expire', 'admin/tank_auth'));
		}
	}
}
